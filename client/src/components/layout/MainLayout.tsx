...
const MainLayout = () => {
  ...
  const title = useMemo(() => {
    ...
    if (
      location.pathname.startsWith("/pronunciation-rules/") &&
      location.pathname !== "/pronunciation-rules"
    ) {
      return "发音规则详情";
    }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-100">
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-6 md:justify-center">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `block rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-primary-50 hover:text-primary-700 ${
                  isActive
                    ? "link-active shadow-sm font-semibold"
                    : "text-slate-600"
                }`
              }
              to={item.to}
      ...
      <div className="flex flex-1 flex-col overflow-y-auto md:ml-64">
        <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm md:px-8">
